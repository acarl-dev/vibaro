...
function LoginForm() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    if (!response.ok) {
      const message = json?.error?.message ?? "Login fehlgeschlagen.";
      setError(message);
      setLoading(false);
      return;
    }
    ...
    const target = apiNext || urlNext || "/studio";
    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-50 flex items-center justify-center px-6">
      <div className="w-full max-w-md rounded-2xl border border-zinc-800 bg-zinc-900/70 p-8 shadow-xl">
        ...
        <p className="mt-2 text-sm text-zinc-400">
          Melde dich mit deiner E-Mail an, um deine Vibaro-Seite zu bearbeiten.
        </p>
        ...
        <p className="mt-6 text-center text-sm text-zinc-400">
          Noch kein Konto?{" "}
          <a href="/register" className="font-medium text-zinc-100 hover:underline">
