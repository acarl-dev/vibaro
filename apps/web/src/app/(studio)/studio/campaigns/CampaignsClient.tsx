        ...
        <div className="space-y-4">
          {campaigns.length === 0 ? (
            ...
            campaigns.map((campaign) => (
              <div
                key={campaign.id}
                className="rounded-lg border border-zinc-700 bg-zinc-800/50 p-6"
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    {campaign.platform && (
                      <p className="mt-1 text-sm text-zinc-400">
                        Plattform: <span className="font-medium text-zinc-300">{campaign.platform}</span>
                      </p>
                    )}

                    {campaign.spotlight_title && (
                      <p className="mt-1 text-sm text-zinc-400">
                        Spotlight: <span className="font-medium text-zinc-300">{campaign.spotlight_title}</span>
                      </p>
                    )}

                    {campaign.notes && (
                      <p className="mt-2 text-sm text-zinc-300">{campaign.notes}</p>
                    )}
                  ...
                  <div className="ml-4 flex gap-2">
                    <button
                      ...
                      className="rounded px-3 py-1.5 text-sm font-medium text-zinc-400 hover:text-zinc-100 transition-colors"
                    >
                      Bearbeiten
                    </button>
                    <button
                      onClick={() => handleDelete(campaign.id)}
                      className="rounded px-3 py-1.5 text-sm font-medium text-red-400 hover:text-red-300 transition-colors"
                    >
                      Löschen
                    </button>
                  </div>
